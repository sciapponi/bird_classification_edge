...
HOST_BIRD_SOUND_DATASET_PREPROCESSED_DIR="$PWD/bird_sound_dataset_processed"
...
CONTAINER_BIRD_SOUND_DATASET_PREPROCESSED_DIR="/app/bird_sound_dataset_processed"
...
REQUIRED_DIRS=(
    ...
    "$HOST_BIRD_SOUND_DATASET_PREPROCESSED_DIR"
    "$HOST_ESC50_DIR"
...
# Volume mounts (with proper quoting for paths with spaces)
...
DOCKER_CMD="$DOCKER_CMD -v \"$HOST_BIRD_SOUND_DATASET_PREPROCESSED_DIR\":\"$CONTAINER_BIRD_SOUND_DATASET_PREPROCESSED_DIR\""
...
# Execute (with proper handling of quoted paths)
eval "$DOCKER_CMD" 